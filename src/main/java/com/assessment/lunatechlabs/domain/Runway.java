...
public class Runway implements Serializable {
	...
	public static Runway from(Map<String, String> values, Function<Long, Long> airportCountryProvider) {
		return builder().id(l(values, "id")).airportId(l(values, "airport_ref"))
				.countryId(airportCountryProvider.apply(l(values, "airport_ref")))
				.airportIdent(s(values, "airport_ident")).lengthFt(s(values, "length_ft"))
				.widthFt(s(values, "width_ft")).surface(s(values, "surface")).lighted(s(values, "lighted"))
				.closed(s(values, "closed")).le_ident(s(values, "le_ident"))
				.le_latitude_deg(s(values, "le_latitude_deg")).le_longitude_deg(s(values, "le_longitude_deg"))
				.le_elevation_ft(s(values, "le_elevation_ft")).le_heading_degT(s(values, "le_heading_degT"))
				.le_displaced_threshold_ft(s(values, "le_displaced_threshold_ft")).he_ident(s(values, "he_ident"))
				.he_latitude_deg(s(values, "he_latitude_deg")).he_longitude_deg(s(values, "he_longitude_deg"))
				.he_elevation_ft(s(values, "he_elevation_ft")).he_heading_degT(s(values, "he_heading_degT"))
				.he_displaced_threshold_ft(s(values, "he_displaced_threshold_ft")).build();
	}
}
